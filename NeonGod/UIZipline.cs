...
namespace NeonGod
{
    internal class UIZipline
    {
        ...
        private static MechController.ZiplinePoint GetZiplinePoint(MechController __instance)
        {
            MechController.ZiplinePoint ziplinePoint = default;
            Vector3 pos = new (0.5f, 0.5f, 0.5f);
            Ray ray = __instance.playerCamera.cam.ViewportPointToRay(pos);

            if (Physics.Raycast(ray, out RaycastHit raycastHit, 800f, __instance.ziplineLayerMask, QueryTriggerInteraction.Ignore))
            {
